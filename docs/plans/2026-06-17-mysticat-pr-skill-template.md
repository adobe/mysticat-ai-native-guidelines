...
<!-- AGENT: Links to supporting material. Include only the ones that exist; when a value is empty, remove the ENTIRE bullet line (do not leave a dangling "- Spec:"). Jira: workspace convention key (e.g. SITES-1234 / LLMO-1234). Look for spec/plan/ADR under docs/ in the touched repos AND in the workspace mysticat-architecture + mysticat-ai-native-guidelines docs (where most specs/ADRs/migration-plans live), keyed on the Jira key / branch / PR title, plus session context. -->
