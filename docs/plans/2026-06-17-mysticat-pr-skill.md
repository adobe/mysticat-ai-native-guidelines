...
- [ ] **Step 3: Resolve the PR body per surface (§A) and allow on the marker.** For `Bash`: read `.tool_input.command`; for `--body/-b` use inline text, for `--body-file/-F <path>` read that file, for `gh api .../pulls -f body=<text|@file>` / `--input <file>` and `curl -d/--data <text|@file>` read the field or referenced file. For `gh api graphql` + `createPullRequest`: resolve the body from the `body:` field of the mutation (inline string or the value of a `-f body=`/`-F body=@file` variable). For MCP: read `.tool_input.body`. If the resolved body contains the marker **key** `mysticat-pr-skill` (match the key, not the literal string - §H may extend the marker with `base`/`head`/`commits`/`files`) -> `exit 0` (allow; skill's own templated output, any transport). (Low real risk on graphql since the skill never emits via graphql, but the rule keeps the recursion guard total across every surface scoped in Step 4.)
...
- [ ] **Step 5: Detect skill presence (§B) - resolve the ACTIVE install, do not bare-glob the cache.** This is the most implementation-fragile step; a naive recursive glob over the plugin cache is wrong. Verified live: the marketplace cache holds multiple versions side by side (`~/.claude/plugins/cache/<marketplace>/review-kit/{1.10.1,1.11.0,1.12.0}/skills/...`), and the authoritative active install is recorded only in `~/.claude/plugins/installed_plugins.json` (`installPath`, with a user-vs-project `scope`). A bare cache glob matches stale version dirs and reports "present" after a downgrade/uninstall, trapping the agent in a deny loop with no installed skill to satisfy it (the documented escape "invoke the skill" then does not exist). Concrete recipe, in order:
  1. Read `~/.claude/plugins/installed_plugins.json`; resolve the active root from `plugins["review-kit@<marketplace>"][].installPath` (honor `scope`); check `<installPath>/skills/create-pr/SKILL.md`.
  2. Also check the project skill dir `<repo>/.claude/skills/create-pr/SKILL.md` and the personal `~/.claude/skills/create-pr/SKILL.md`.
  3. Fall back to a cache glob ONLY if `installed_plugins.json` is absent/unreadable; note that multiple cached version dirs coexist, so a bare glob is NOT authoritative.
  Any of (1)-(2) present -> present. Keep it cheap (file stat + one small JSON read; no network, no tree walk). The Task 2.3 "skill present" fixture MUST exercise this exact recipe (stale cache dir present but `installed_plugins.json` points elsewhere) so the test cannot pass while real detection diverges.
- [ ] **Step 6: Decision (§A decision table).** No marker + create attempt + skill present -> deny. No marker + create attempt + skill absent -> `exit 0` + stderr warning (allow fallback). Unresolvable `--body-file` (missing/unreadable) -> fail-safe: deny when the skill is present (degrade to the raw-call path, never a silent allow). The deny `message` is the entire nudge-reliability lever, so pin the literal recommended string (and surface both escape hatches in it, so the override is discoverable at the moment of friction):
  > `Open this PR with the create-pr skill (it fills the team PR template). To create a raw PR instead, set MYSTICAT_PR_SKILL_BYPASS=1, or run gh pr create --web to fill the body in the browser.`
  Keep it imperative and naming the skill; adjust wording but retain the skill name plus the two escape hatches.
...
- [ ] **Step 1b: Skill-presence fixtures (pins Step 5's recipe).** Stale cache version dir present but `installed_plugins.json` `installPath` points elsewhere (or is absent) -> NOT present (no deny loop); active `installPath` contains `skills/create-pr/SKILL.md` -> present; project/personal skill dir contains it -> present. The "skill present" stub used by the deny/allow cases above MUST resolve presence via the same recipe, so a test cannot pass while real detection diverges.
...
- [ ] Hook allows raw creation + stderr warning when the skill is absent; clean no-op on unrelated Bash.
- [ ] Latency bound (the hook runs on every Bash call): at most a file stat plus one small JSON read (`installed_plugins.json`) on the create path, and an early `exit 0` with no I/O on the no-op path. No network, no clone, no filesystem tree walk.
...
- Label reading: `src/worker/steps/label.py` is write-only (`add_label`). §E defines TWO equivalent exemption channels - a `no-spec-needed` label OR an `Exemption:` bullet - and the posted review body carries neither for a label-only exemption. So a body-only `detect_grounding` would misclassify a label-exempt PR as `spec_found` (the exact bucket §G's split tries to isolate). **Decision: add a `list_labels` helper** (mirror `add_label`'s `gh api repos/{owner}/{repo}/issues/{n}/labels` shape; the PR repo is reached via `gh api`, no clone - worker clones only `mysticat-workspace` + 3 context repos + skills repo per `workspace.py` `_CONTEXT_REPOS`) so both §E channels map to `exempt`. (If a later decision drops the label channel, document that label-only exemptions register as `spec_found`; do not leave it ambiguous.)
...
- [ ] **Step 1: `detect_grounding(body, labels)`** in `parser.py` - reuse the fence-skipping loop. Precedence: `exempt` when EITHER the `no-spec-needed` label is present (from the `list_labels` helper above) OR an `Exemption:` bullet is in the body; else `degraded` on the `⚠ Degraded review` banner; else `spec_found`. None-safe (`if not body and not labels: return ...`). Passing labels in (rather than body-only) is what lets a label-only exemption resolve to `exempt` instead of silently folding into `spec_found` (Important review item). **Cross-repo string contract:** this parser keys on the literal banner prefix and `Exemption:` bullet that Part 3A emits in the other repo, with no shared constant - so treat the banner text and bullet wording as tokens of record. Any change to Part 3A's banner/exemption wording is a breaking change to this parser even though the two halves deploy independently; the runtime decoupling does not remove this coupling. Pin both literals in the `test_parser.py` cases below so a drift fails a test.
...
- [ ] **Step 3: Wire the pipeline.** After `count_findings`, fetch the PR labels via the `list_labels` helper and compute `grounding = detect_grounding(posted["body"], labels)`; pass `grounding` to `emit_amp_success`. **AMP-only (firm decision, do not reopen):** emit the `grounding` label on the AMP cost + input-token series only - NOT as a CloudWatch dimension - matching the `review_state` precedent and `metrics.yaml`, so CloudWatch dimension cardinality does not grow. Optionally include `grounding` in the Check Run metadata dict (no cardinality cost there).
...
- [ ] **Step 6: Tests** - `test_parser.py` for `detect_grounding` covering all four inputs: banner-present -> `degraded`; `Exemption:` bullet -> `exempt`; `no-spec-needed` label only (no bullet, no banner) -> `exempt`; none -> `spec_found` (pin the literal banner prefix + `Exemption:` bullet per Part 3B Step 1). `test_amp.py` for the label on the wire + the unknown-label guard still green; `test_pipeline.py` for label-fetch + threading into `emit_amp_success`; `test_metrics_catalog.py` stays green (names unchanged). Run `pytest` -> Expected: all pass.
...
- [ ] Label-only-exempt PR (a `no-spec-needed` label, no `Exemption:` bullet, no banner) -> `detect_grounding` returns `exempt` (not `spec_found`), pinned in `test_parser.py` - so a §E label-channel exemption is not silently miscounted.
