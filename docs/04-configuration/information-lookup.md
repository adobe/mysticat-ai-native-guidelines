...
1. **Check the cache first.** Look for a prior synthesis on this topic in your research cache (a gitignored scratch path). If found and fresh (say, under ~2 weeks old), offer to reuse it; if older, default to re-running. Age is the floor: also re-run when the user signals freshness ("fresh", "from scratch") or when you already know from other work that a key source changed (ticket reopened, decision reversed, PR merged). Don't re-fan just to check whether something changed; that defeats the cache.
